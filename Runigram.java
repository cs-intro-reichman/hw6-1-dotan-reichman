...
public class Runigram {
    ...
    public static Color[][] flippedHorizontally(Color[][] image) {
        ...
        Color[][] rtr = new Color[image.length][image[0].length];
        for (int i = 0; i < image.length; i++) {
            for (int j = 0; j < image[i].length; j++) {
                rtr[i][j] = image[i][image[i].length - j - 1];
    ...
    public static Color[][] flippedVertically(Color[][] image) {
        Color[][] rtr = new Color[image.length][image[0].length];
        for (int i = 0; i < image.length; i++) {
            for (int j = image[i].length - 1; 0 < j; j--) {
                rtr[i][j] = image[image.length - 1 - i][j];
            }
        }
        return rtr;
    }
    ...
    private static Color luminance(Color pixel) {
        int rred = pixel.getRed();
        int rgreen = pixel.getGreen();
        int rblue = pixel.getBlue();
        double lum = 0.299 * rred + 0.587 * rgreen + 0.114 * rblue;
        int lumrtr = (int) lum;
        Color lum1 = new Color(lumrtr, lumrtr, lumrtr);
        return lum1;
    }
    ...
    public static Color[][] grayScaled(Color[][] image) {
        Color[][] greyrtr = new Color[image.length][image[0].length];
        for (int i = 0; i < image.length; i++) {
            for (int j = 0; j < image[i].length; j++) {

                greyrtr[i][j] = luminance(image[i][j]);
            }
        }
    ...
    public static Color[][] scaled(Color[][] image, int width, int height) {
        ...
        for (int i = 0; i < scaled.length; i++) {
            for (int j = 0; j < scaled[i].length; j++) {
                scaled[i][j] = image[image.length * i / height][image[0].length * j / width];
            }
        }
        return scaled;
    }
    ...
    public static Color blend(Color c1, Color c2, double alpha) {
        ...
        int g2 = c2.getGreen();
        int r2 = c2.getRed();
        int rred = (int) (alpha * r1 + (1 - alpha) * r2);
        int rblue = (int) (alpha * b1 + (1 - alpha) * b2);
        int rgreen = (int) (alpha * g1 + (1 - alpha) * g2);
        Color result = new Color(rred, rgreen, rblue);
        return result;
    }
    ...
    public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
        Color[][] rtr = new Color[image1.length][image1[0].length];
        for (int i = 0; i < image1.length; i++) {
            for (int j = 0; j < image1[i].length; j++) {
                rtr[i][j] = blend(image1[i][j], image2[i][j], alpha);
            }
        }
        return rtr;
    }
